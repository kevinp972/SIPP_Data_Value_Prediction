...
# this part of code is from the TA
library(MASS)
library(glmnet)
n <- nrow(data)
k <- 10 
...
# ## tw ~ fsize
# ggplot(data = data) +
#   geom_point(mapping = aes(x = fsize, y = tw)) ## when goes above 5, it gets low, splines
# 
# summary(data$fsize)
# 
...
# ii <- sample(rep(1:k, length = n))
# 
# ## max number of df, p = 10, 10 knots for ns
# p <- 10
# mspe_knots <- rep(NA, p)
# for (deg in 1:p) {
...
#     train <- (ii != f)
# 
#     # Cubic spline fit with knots at kn
#     fsizereg <- lm(tw ~ ns(fsize, df = deg), data=data[train,])
# 
#     # Make predictions
#     pred <- predict(fsizereg, newdata = data[test,])
# 
#     # Calculate MSPE
#     mspe[f] <- mean((data$tw[test] - pred)^2)
#   }
#   mspe_knots[deg] <- mean(mspe)
# }
# plot(mspe_knots)
# which.min(mspe_knots)
# ## the warning is caused by a limited x size and it being a discrete variable
# 
# ## plotting it
# x.grid <- seq(from = min(data$fsize), to = max(data$fsize), length = 7933)
# fsize_spline <- lm(tw ~ ns(fsize, knots=c(2,3,4,8,9)), data = data)
# fsize_splinedefault <- lm(tw ~ ns(fsize, df = 5), data = data)
# pr1 <- predict(fsize_spline, newdata = list(fsize = x.grid))
# pr2 <- predict(fsize_splinedefault, newdata = list(fsize = x.grid))
# ggplot(data = data) +
#   geom_point(mapping = aes(x = fsize, y = tw)) +
#   geom_line(mapping = aes(x=x.grid, y = pr1), lwd = 1, color = "Blue") +
#   geom_line(mapping = aes(x=x.grid, y = pr2), lwd = 1, color = "Green")
# 
# ## k-fold cross validation to test custom knots vs default knots
# n <- nrow(data)
# k <- 10
# set.seed(123)
# id <- sample(rep(1:k, length = n))
# mspe_fsize_spline <- mspe_fsize_splinedefault <- mspe_fsize_linear <- rep(NA, k)
# 
# for (f in 1:k){
#   test <- (id == f)
#   train <- (id != f)
#   fsize_spline <- lm(tw ~ ns(fsize, knots=c(2,3,4,8,9)), data = data[train,])
#   fsize_splinedefault <- lm(tw ~ ns(fsize, df = 5), data = data[train,])
#   fsize_linear <- lm(tw ~ fsize, data = data[train,])
#   prfspline <- predict(fsize_spline, newdata = data[test,])
#   prfsplinedefault <- predict(fsize_splinedefault, newdata = data[test,])
#   prflinear <- predict(fsize_linear, newdata = data[test,])
#   mspe_fsize_spline = mean((data$tw[test] - prfspline)^2)
#   mspe_fsize_splinedefault = mean((data$tw[test] - prfsplinedefault)^2)
#   mspe_fsize_linear = mean((data$tw[test] - prflinear)^2)
# }
# 
# c(mspe_fsize_linear, mspe_fsize_splinedefault, mspe_fsize_spline)
# 
# ## now we add the splines of fsize to our baseline model(GAM)
# library(splines)
# 
# fsize.spline <- ns(data$fsize, knots=c(2,3,4,8,9))
# fsize.spline <- as.data.frame(fsize.spline)
# # This gives names to fsize.spline before merging with the original dataset.
# # This will ensure the names are given by fsize_spline# rather than some arbitrary naming scheme.
# names(fsize.spline) <- paste0("fsizespline", 1:6) ## # of knots + 4 degrees of freedom
# 
# # Combine dataset with fsize.spline
# data_transform <- cbind(data_transform, fsize.spline)
# 
# # Remove fsize from transformed data
# data_transform <- subset(data_transform, select = -fsize)
# dim(data_transform)
# 
# ## cross validation for ridge lasso and step for fsize transformed data
# set.seed(123)
# library(MASS)
# library(glmnet)
# 
...
# k <- 10
# ii <- sample(rep(1:k, length= n))
# pr.stepwise_backward <- pr.stepwise_forward <- pr.lasso <- pr.ridge <- rep(NA, nrow(data_transform))
# 
# for (j in 1:k){
# 
#   hold <- (ii == j)
...
#   pr.stepwise_backward[hold] <- predict(b, newdata=data_transform[hold,])
#   pr.stepwise_forward[hold] <- predict(a, newdata=data_transform[hold,])
# 
#   ## Do with lasso (we use X and y defined above)
#   xx.tr <- data_transform[train,-1]
#   y.tr <-  data_transform$tw[train]
#   xx.te <- data_transform[hold,-1]
...
# }
# 
# mspe_step_backward <- mean((pr.stepwise_backward-data_transform$tw)^2)
# mspe_step_forward <- mean((pr.stepwise_forward-data_transform$tw)^2)
# mspe.Lasso <- mean((pr.lasso-data_transform$tw)^2)
# mspe.ridge <- mean((pr.ridge-data_transform$tw)^2)
# 
# ## Check the best
# c(mspe.ridge, mspe.Lasso, mspe_step_forward, mspe_step_backward)
